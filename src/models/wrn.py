...
##########################################################################################
# Wide ResNet (WRN-16-4) – taken verbatim from Hendrycks et al. Outlier-Exposure repo
##########################################################################################
...
class NetworkBlock(nn.Module):
    ...
    def _make_layer(self, block, in_planes, out_planes, nb_layers, stride, dropRate):
        layers = [
            block(i == 0 and in_planes or out_planes,
                  out_planes,
                  i == 0 and stride or 1,
                  dropRate)
...
class WideResNet(nn.Module):
    def __init__(self, depth: int = 16, widen_factor: int = 4,
                 num_classes: int = 10, dropRate: float = 0.0):
        super().__init__()
        nChannels = [16,
                     16 * widen_factor,
                     32 * widen_factor,
